...
char server[] = "192.168.0.143";
char dataStr[12];
String hasildata;
char c;
int x = 0;
int pinrelay = 4, pinreset = 7;
String qstring2;
  ...
  for (count = 0; count < 5; count++) {
    if (wg.available())
    {
      ...
      if (client.connect(server, 80)) {
        ...
        client.println("GET /access-app/api/data" + queryString + " HTTP/1.1");
        client.println("Host: 192.168.0.143");
...
void loop() {
  while (client.connected() || client.available())
  {
    ...
    delay(500);
    bacadata(); //print the character that has been captured by array at Serial Monitor
  }
